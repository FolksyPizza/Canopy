...
public class BackendPlaySessionHandler implements MinecraftSessionHandler {
  ...
  public void handleUnknown(ByteBuf buf) {
    ...
    ByteBuf out = rewriter.active(serverConn.getPlayer().getProtocolVersion())
        ...
        : buf;
    boolean rewritten = out != buf;
    boolean huge = out.readableBytes() > LARGE_PACKET_THRESHOLD;
    // Unchanged: retain the original (the pipeline releases its own ref after this call). Rewritten:
    // `out` is a fresh buffer we own, so write it directly and let the pipeline release the original.
    playerConnection.delayedWrite(rewritten ? out : out.retain());
    if (huge || ++packetsFlushed >= MAXIMUM_PACKETS_TO_FLUSH) {
      playerConnection.flush();
      packetsFlushed = 0;
    }
