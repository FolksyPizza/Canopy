...
public class ClientPlaySessionHandler implements MinecraftSessionHandler {
  ...
  public void handleUnknown(ByteBuf buf) {
    ...
    final boolean stateAllowsForward = smc != null
        ...
        && smc.getState() == StateRegistry.PLAY;
    if (stateAllowsForward) {
      // Translate the player's own entity id (and any collision partner) in serverbound packets
      // so the backend sees its own id space. No-op unless no-respawn mode is active.
      if (canopyRewriter.active(player.getProtocolVersion())) {
        ByteBuf out = canopyRewriter.processServerbound(buf);
        smc.write(out != buf ? out : out.retain());
      } else {
  ...
  /**
   * Experimental seamless switch that sends no JoinGame or Respawn, so the client keeps its current
   * world and never shows the "loading terrain" screen. It only works when source and destination
   * are the same dimension (the case Canopy always produces: one world split across shards).
   *
   * <p>Two things a Respawn would normally have handled are done explicitly: every entity the
  ...
  private void doNoRespawnClientServerSwitch(JoinGamePacket joinGame) {
    ...
    if (stale.length > 0) {
      io.netty.buffer.ByteBuf destroy = player.getConnection().getChannel().alloc().buffer();
